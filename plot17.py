...
MLRUNS_PATH = "mlruns2/0"
OUTPUT_FILE = "hyperparameter_analysis_uil.xlsx"
...
for run_id in os.listdir(MLRUNS_PATH):
    ...
    try:
        ...
        record = {
            ...
            "unlearning_rate": float(params["unlearning_rate"]),
...
# Dla każdej kombinacji (window_size, unlearning_rate, learning_rate)
# obliczamy średnią accuracy.

df_mean = (
    df.groupby(
        ["window_size", "unlearning_rate", "learning_rate"],
        as_index=False,
    )["accuracy"]
    .mean()
)
...
try:
    interaction_model = ols(
        "accuracy ~ C(window_size) * C(unlearning_rate) * C(learning_rate)",
        data=df_interaction,
    ).fit()
